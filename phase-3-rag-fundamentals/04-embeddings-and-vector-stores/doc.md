...
vector_store = Chroma.from_documents(chunks, embedding=embeddings)  # Phase 0 Topic 06
```

`Chroma.from_documents` does two things in one call: embeds every chunk's
...
[vector, vector, vector, ...]  (one per chunk, metadata + text kept alongside)
        │
        ▼  stored + indexed in Chroma            <- Phase 0 Topic 06
        ▼
query -> embeddings.embed_query(...) -> nearest-neighbor search -> top-k Documents
```

By default `Chroma()` without a `persist_directory` keeps everything in memory — gone
when the process exits, which is why every topic so far rebuilds its store on each run.
Passing `persist_directory="./chroma_db"` writes the index to disk so a later process
can reopen it without re-embedding anything (re-embedding costs API calls and time).

## Minimal code

`code.py` splits a document into chunks (Topic 03's splitter), embeds them into a
**persisted** Chroma store on disk, runs a similarity search, then reopens the *same*
on-disk store in a fresh `Chroma(...)` call (no `.from_documents`, no re-embedding) and
proves the data survived — the actual behavior that makes persistence worth doing.
...
- Reopening a persisted store returns 0 results → check `persist_directory` points to
  the exact same path used when the store was created — a relative path resolved from a
  different working directory silently points at an empty/new directory.
...
Run `code.py` twice in a row without deleting the `chroma_db` folder it creates, and
